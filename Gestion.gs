...
/**
 * Envoie par e-mail un lien d'accès à l'Espace Client pour l'adresse fournie.
 * Utilise un lien signé si activé, sinon un lien simple vers la page gestion.
 * @param {string} emailClient
...
function envoyerLienEspaceClient(emailClient) {
  try {
    ...
    let burstCount = 0;
    try { burstCount = parseInt(cache.get(burstKey) || '0', 10) || 0; } catch (_e) { burstCount = 0; }
    ...
    try {
      if (typeof CLIENT_PORTAL_SIGNED_LINKS !== 'undefined' && CLIENT_PORTAL_SIGNED_LINKS) {
        const lien = generateSignedClientLink(email);
        url = lien && lien.url ? lien.url : '';
        expTs = lien && lien.exp ? Number(lien.exp) : 0;
      }
    } catch (_err) {
      url = '';
    }
    if (!url) {
      ...
      url = baseUrl + '?page=gestion&email=' + encodeURIComponent(email);
    }

    const logoBlock = (typeof getLogoEmailBlockHtml === 'function') ? getLogoEmailBlockHtml() : '';
    var expText = '';
    if (expTs) {
      try { expText = ' (valable jusqu\'au ' + new Date(expTs * 1000).toLocaleString('fr-FR') + ')'; } catch (_e) {}
    }
    const corpsHtml = [
      '<div style="font-family: Montserrat, sans-serif; color: #333;">',
      logoBlock || '',
      '<h1>Votre lien d\'accès à l\'Espace Client</h1>',
      '<p>Bonjour,</p>',
      '<p>Utilisez le lien suivant pour accéder à votre espace client' + expText + ' :</p>',
      '<p><a href="' + url + '">' + url + '</a></p>',
      '<p>Si vous n\'êtes pas à l\'origine de cette demande, vous pouvez ignorer cet e-mail.</p>',
      '</div>'
    ].join('');

    const subject = 'Lien d\'accès - ' + (typeof NOM_ENTREPRISE !== 'undefined' ? NOM_ENTREPRISE : 'Votre service');
    GmailApp.sendEmail(
      email,
      subject,
      'Votre lien d\'accès est disponible dans la version HTML.',
      { htmlBody: corpsHtml, replyTo: (typeof EMAIL_ENTREPRISE !== 'undefined' ? EMAIL_ENTREPRISE : '') }
    );
    return { success: true };
  } catch (e) {
    Logger.log('envoyerLienEspaceClient error: ' + e.stack);
